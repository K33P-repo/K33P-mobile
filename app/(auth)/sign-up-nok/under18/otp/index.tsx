...
export default function OTPEntryScreen() {
  ...
  const { nokPhoneNumber, nokFormattedNumber } = useNokPhoneStore();

  const triggerSendOTP = async () => {
    if (!nokPhoneNumber) return;
    try {
      setIsLoading(true);
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      await verifyOTP(nokPhoneNumber, otpString);
      ...
      <View className="flex-1 px-5 ">
        ...
        <View className="flex-1">
          ...
          <Text className="text-neutral200 font-sora text-sm text-center mb-8 px-8 py-2">
            A 5-digit OTP has been sent to {nokFormattedNumber}
          </Text>
