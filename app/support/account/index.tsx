...
export default function AccountHelpScreen() {
  ...
  // Handle focus - don't navigate, just let user type
  const handleFocus = () => {
    setIsKeyboardVisible(true);
    // Don't navigate on focus anymore
  };

  const renderAccountContentItem = ({ item }: { item: any }) => (
    <View className="mb-6">
      {item.heading && (
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* Search bar - only navigates when there's text */}
            ...
            onChangeText={(text) => {
              ...
              if (text.trim().length > 0) {
                router.push({
                  ...
                  params: { query: text, from: 'account' }
                });
              }
            }}
            onFocus={handleFocus}
