...
// Separated so its ScrollView doesn't share a responder chain with the
// parent horizontal FlatList (which was stealing vertical touches).
const SearchResultCard = React.memo(({
  ...
  >
    ...
    {/* Plain ScrollView — eliminates the nested FlatList vs parent FlatList
        touch conflict that caused laggy / multi-swipe vertical scrolling */}
    <ScrollView
      style={{ flex: 1 }}
...
export default function AccountHelpScreen() {
  ...
  const accountContent = helpContent.helpSections.find(
  ...
  // Reset to first card whenever the result set changes
  useEffect(() => {
    setSearchCurrent(0);
  }, [searchResults]);
  ...
  const renderAccountContentItem = useCallback(({ item }: { item: any }) => (
    <View className="mb-6">
      {item.heading && (
      ...
      {searchQuery ? (
        searchResults.length > 0 ? (
          <View className="flex-1">
            {/*
              scrollEnabled={false} is the key change:
              All horizontal movement is now driven exclusively by the arrow
              buttons — no touch-based swiping at all, so vertical touches
              inside the card are never intercepted by this list.
            */}
            <FlatList
        ...
        <View className="flex-1 px-4">
          <View className="mt-4 p-4 rounded-lg bg-[#222222] flex-1">
            ...
            <FlatList
              data={accountContent.content}
              keyExtractor={item => `account-${item.id}`}
              renderItem={renderAccountContentItem}
