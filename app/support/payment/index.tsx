...
export default function PaymentHelpScreen() {
  ...
  const paymentContent = helpContent.helpSections.find(
    section => section.section === 'Payment'
  );
  ...
  const renderPaymentContentItem = ({ item }: { item: any }) => (
    <View className="mb-6">
      {item.heading && (
            ...
            onChangeText={(text) => {
              ...
              if (text.trim().length > 0) {
                router.push({
                  ...
                  params: { query: text, from: 'payment' }
                });
              }
            }}
            onFocus={handleFocus}
