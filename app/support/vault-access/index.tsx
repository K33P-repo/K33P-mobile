...
// ─── Reusable isolated card component (same as in other screens) ──────────────
...
export default function VaultAccessHelpScreen() {
  ...
  const vaultContent = helpContent.helpSections.find(
    (section) => section.section === 'Vault Access'
  );
  ...
  const searchResults = useMemo(() => {
              ...
              {c.id === 1 ? (
                <>
                  <Text className="text-main">
                    {highlightText('Loss of device - ', searchQuery)}
                  </Text>
    ...
    // Prioritize Vault Access section first
  ...
  const renderVaultContentItem = useCallback(({ item }: { item: any }) => (
    <View className="mb-6">
      {item.heading && (
        ...
        <View className="flex-1 px-4">
          <View className="mt-4 p-4 rounded-lg bg-[#222222] flex-1">
            <Text className="text-neutral100 text-xs font-space-mono mb-4">
              About K33P Vault access & Recovery
            </Text>
            <FlatList
              data={vaultContent.content}
              keyExtractor={(item) => `vault-${item.id}`}
              renderItem={renderVaultContentItem}
